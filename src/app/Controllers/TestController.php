...
class TestController extends Controller {
	...
	public function getGeneralTest() {
        if(\App::environment('local')){
            ...
            $items = \Solunes\Pagostt\App\FieldTranslation::where('label', 'like', '%fields.%')->groupBy('label')->orderBy('label')->get();
            if(count($items)>0){
            ...
            $items = \Solunes\Pagostt\App\FieldOptionTranslation::where('label', 'like', '%admin.%')->groupBy('label')->orderBy('label')->get();
            if(count($items)>0){
            ...
            $items = \Solunes\Pagostt\App\MenuTranslation::where('name', 'like', '%admin.%')->groupBy('name')->orderBy('name')->get();
            if(count($items)>0){
            ...
            $items = \Solunes\Pagostt\App\Field::whereIn('parent_id', $nodes_array)->where('type', 'text')->get();
            if(count($items)>0){
